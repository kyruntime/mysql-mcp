...
シンプルな設定、すぐに使える、高速で安全。自然言語でデータベースと対話できます。
...
- **mysql_use_database** — 現在のデータベースを切り替え（`USE db_name`）
- **mysql_export_csv** — テーブルデータまたはクエリ結果を CSV でエクスポート
...
- [x] マルチデータベース切り替え（`USE db_name`）
- [x] テーブルデータの CSV エクスポート
- [ ] スロークエリの自動分析と最適化提案
